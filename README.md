...
## Phase 0 — Java 21 Refresh for Kotlin Devs

**Goal:** Read any Java 21 Spring codebase without friction. Write idiomatic modern Java when the ecosystem demands it.

**Hours:** 10 hr • **Calendar:** Week 1

### Core Concepts

- Java 21 LTS features: `record`, sealed classes/interfaces, pattern matching (`switch` expressions, `instanceof`), `var` (local inference), text blocks
- **Virtual threads** (Project Loom) — the big one for Spring Boot 3.2+
- Streams API (post-Kotlin collections, you'll find it clunky — that's fine, just read fluency)
- `Optional<T>` — pragmatic usage, not theology
- Checked exceptions — how to survive them, when to wrap
- Java module system (JPMS) — read-level only; Spring Boot mostly ignores it
- Modern build: Maven lifecycle (since 80% of Spring tutorials use it) + Gradle Kotlin DSL for your own work

### Android/Kotlin Analog

| Kotlin | Java 21 | Notes |
|---|---|---|
| `data class User(val name: String)` | `record User(String name)` | Records are final and immutable |
| `sealed class Result` | `sealed interface Result permits Success, Failure` | Same idea, verbose |
| `when (x) { is A -> ... }` | `switch (x) { case A a -> ... }` | Pattern matching arrived in 21 |
| `val x = 1` | `var x = 1` | Only local inference in Java |
| `"""text"""` | `"""text"""` | Identical |
| `runCatching { }` | `try { } catch { }` | No idiomatic Result type; use Spring's `ResponseEntity` or custom |
| `suspend fun` on IO dispatcher | Virtual thread on `Thread.ofVirtual().start { }` | Same "cheap-thread" model, no colored functions |

### ⚠️ Traps

> **Trap 1 — Streams are not sequences.** `.stream()` is single-use; Kotlin `Sequence` is reusable. Reusing a consumed stream throws at runtime.

> **Trap 2 — `Optional` is not a null-safety system.** It's a return-type convention. Fields/params should not be `Optional`. Don't go Kotlin-coder-angry on missing null-safety — just enable NullAway/SpotBugs and move on.

> **Trap 3 — Equals/hashCode on mutable classes.** Kotlin `data class` copies are cheap; Java beans pre-records require manual overrides. Records solve this, but you'll see legacy code.

### Hands-On

Port one small Kotlin utility from your Android code (say, a CSV parser or a date-range calculator) to idiomatic Java 21 using records + pattern matching + streams. Don't use Kotlin. Feel the friction.

### Best Resources

- **Inside Java YouTube channel** (Nicolai Parlog, Jose Paumard) — 10–20 min videos on records, virtual threads, pattern matching. Watch: "Java Records in Practice", "Virtual Threads Explained"
- **Baeldung — "Java 21 Features" guide** — cherry-pick what's new since 11
- **Book (optional):** "Modern Java in Action" (Urma/Fusco/Mycroft) — **skip chapters 1–3** (you already know this), read chapters on streams, Optional, CompletableFuture
- **Skip:** entire courses on "Java for beginners", OOP fundamentals, generics tutorials, collection framework walkthroughs

### Skip This

- Generics variance deep-dive (`? extends` / `? super`) — read the one-pager, it maps to Kotlin `out/in`
- Manual getters/setters/builders — records eliminate 80% of this
- JVM internals (classloaders, GC tuning) — not needed until prod debugging, Phase 4
- Swing/AWT, JavaFX — irrelevant
- JDBC raw API — you'll use JPA
- Servlet API deep-dive — Spring abstracts it

### Exit Criteria

- [ ] Can read a Spring `@Service` class with records, `var`, pattern-matching switch without looking anything up
- [ ] Can explain virtual threads vs platform threads vs coroutines in 3 sentences each
- [ ] Can set up a new Java 21 Gradle project with one command and a `build.gradle.kts`
- [ ] Can navigate a Maven `pom.xml` well enough to add a dependency without googling
